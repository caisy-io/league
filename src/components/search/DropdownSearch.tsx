...
import { SDropdownOption, SOptionHeader, SDropdownSearch } from "../oldsearch/styles/SSearchWithFilter";
...
const DropdownSearch = ({
  ...
  const createDropdownOptions = () => {
    ...
    const typeOptions = optionsData
      .map((option: any) => ({ ...option, section: type }))
    ...
    const fieldOptions = optionsData
      .map((option: any) => ({ ...option, section: field }))
      .filter((item) => item.__typename === "SchemaField")
      .filter((item) => item.title.toLowerCase().includes(searchResult.toLowerCase()));

    const allOptions: any = searchLevel == ISearchLevels.level1 ? typeOptions : fieldOptions;
